...
% below it. The same box the typst masthead draws: a three-quarter-point rule
% 6pt clear of the text on every side, with 9pt above and below.
...
% The author note, across the foot of the first page under a rule.
%
...
% does both. Here it always spans the page, because the column is not open to
% a note of any length: latex will only set a footnote that fits in the column
% it was raised in, and the fifteen lines example.qmd's note runs to does not,
% which sent the output routine round in circles rather than breaking it.
%
...
%
% None by default. A manuscript is double spaced, and the leading alone opens
% more room there than APA asks for. A journal article is single spaced, and
% the note's first line came within half a point of the rule: it read as the
% last row of the table rather than as a note under it. formatlatex.lua sets
% the length for jou, to the half em that brings it to what apa7 leaves.
%
