...
if (file.exists("d_env.csv")) {
  d_env <- read_csv("d_env.csv")
} else {
  d_env <- tibble(label = character(0),
                title = character(0), 
                caption = character(0),
                env = character(0),
...
#            env = "Figure",
#            extension = ".svg",
#            filename = paste0(label, extension)) {
#     envtext <- env
#     str_label <- label
#     d_env <<- d_env |> filter(!(label == str_label)) |> unique() 
#     dbl_possible <- d_env |> 
#       filter(env == envtext) |> 
#       nrow() %>%
#       `+`(1) |> 
#       seq()
#     
#     dbl_present <- d_env %>% filter(env == envtext) %>% 
#                     separate(title, c("env2", "env_num")) |>
#                      pull(env_num) |> 
#                      as.numeric() |> 
#       unique()
#     
#     env_number <- dbl_possible[!(dbl_possible %in% dbl_present)] |> 
#       min()
#     d_env <<- add_row(
#       d_env,
#       label = label,
#       title = paste0(env, " ", env_number),
#       caption = caption,
#       env = env,
#       filename = filename,
#       note = note
...
# ref_env <- function(l) {
#   d_env %>% filter(label == l) %>% pull(title)
# }
...
if (interactive()) {
  yml_metadata <- rmarkdown::yaml_front_matter("asdf.qmd")
} else {
  yml_metadata <- rmarkdown::metadata
}

d_author_affiliation <-
  yml_metadata$author |> 
   map(\(x) if (is.character(x) & length(x) == 1) list(name = x) else x) %>% 
   map_df(as_tibble) %>% 
    mutate(affiliation = map(affiliation, \(x) if (is.character(x) & length(x) == 1) list(affiliation = x) else x)) %>% 
    bind_rows(
      tibble(
        id = character(0),
        number = integer(0),
  ...
  mutate(affiliation = map(affiliation, enframe)) |> 
  unnest(affiliation) %>% 
   mutate(value = as.character(value),
          name = factor(name, 
                        levels = c("id", 
                                   ...
                                   "country",
                                   "url"))) |> 
   pivot_wider(names_expand = TRUE, names_prefix = "affiliation_") %>% 
    mutate(affiliation_name = ifelse(is.na(affiliation_name), affiliation_affiliation, affiliation_name)) %>% 
  unite(regioncode, affiliation_region, `affiliation_postal-code`, remove = F, sep = " ", na.rm = TRUE) %>% 
  unite(affiliation_default, affiliation_city, affiliation_region, affiliation_country, na.rm = TRUE, remove = F, sep = ", ") %>% 
  unite(mailing, affiliation_address, affiliation_city, regioncode, affiliation_country, sep = ", ", na.rm = TRUE) %>% 
  unite(affiliation, affiliation_department, affiliation_name, sep = ", ", na.rm = T) %>% 
  unique() %>% 
  nest(.by = affiliation) %>% 
  mutate(affiliation_row = row_number()) %>% 
  unnest(data) %>% 
  mutate(affs = paste0(affiliation_row, collapse = ","),.by = author) %>% 
  mutate(max_author = max(author_id, na.rm = TRUE),
         max_affiliation = max(affiliation_row,na.rm = TRUE),
         ...
         affiliation = ifelse(
           ...
           ifelse(
             is.na(affiliation_default) | affiliation_default == "",
             "Unaffiliated",
             affiliation_default
             ), 
           affiliation),
         affiliation_display = ifelse(count_affs == 1 | max_author == 1, affiliation, paste0("^",affiliation_row, "^", affiliation))) 

display_authors <- d_author_affiliation %>% 
  pull(author_display) %>% 
  unique() %>% 
  knitr::combine_words()

display_affiliations <- d_author_affiliation %>% 
  pull(affiliation_display) %>% 
  unique() %>% 
  paste0(collapse = "\n\n") 
...
display_orcids <-  d_author_affiliation %>% 
  select(author, orcid) %>% 
  unique() %>% 
  filter(!orcid == "NULL") %>% 
  mutate(orcid_display = paste0(
    author, 
    " ![](ORCID-iD_icon-vector.svg){width=16px} https://orcid.org/",
    orcid)) %>% 
  pull(orcid_display) %>% 
  unique() %>% 
  paste0(collapse = "\n\n")

display_corresponding_author <- d_author_affiliation %>% 
...
library(officer)
read_docx("apa_pristine.docx") %>%
  headers_replace_all_text("Running Header", str_to_upper(yml_metadata$runningheader)) %>%
