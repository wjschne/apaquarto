    ...
    if (str_starts(options$label, pattern = "apafg-") |
      ...
      if (before) {
        ...
        if (is_pdf) {
          pre <- ""
          # if (apatype == "figure") {
          #   if (is.null(options$`fig.pos`)) fgpos <- "!h" else fgpos <- options$`fig.pos`
          #   message(fgpos)
          #   pre <- paste0(pre, "\n\\begin{figure}[",fgpos,"]\n")
          #   fgalign <- knitr::opts_current$get("fig.align")
          #   if (is.null(fgalign)) {
          #     fgalign <- ""
          #     } else {
          #       if (fgalign == "left") {
          ...
          #       }
          #     }
          #   pre <- paste0(pre, fgalign)
          # 
          # }
          # options$`fig-cap` <- options$`apa-cap`
