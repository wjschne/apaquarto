...
### Notes in a Document That Is Not in an apaquarto Format

`apa-note` is read by one of apaquarto's filters rather than by Quarto, so a
document in a plain format ignores it and the note simply does not appear. A
document that wants the notes without the rest of APA style can ask for those
filters by name:
...
That is the whole of it. `apanote` is a small companion extension that is
installed alongside apaquarto and names a few of its filters. Nothing else of
apaquarto comes with them: no title page, no APA headings, no stylesheet. The
note is written in place below its figure or table and takes whatever styling
the document already has.

Notes on figures and tables written in code chunks, on markdown images, and on
markdown tables are all read. Which of them survive depends on the format:
...
A document already in an apaquarto format has these filters and does not need
`apanote` --- though if it names it anyway, each note is still written once.
