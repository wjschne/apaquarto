...
| `html`, `revealjs` | all |
| `docx` | all |
| `typst` | figures only; a note on a markdown table is lost, because Quarto's typst writer has already turned the table into typst before the filter runs |
| `pdf` | not supported; notes in .pdf need the LaTeX machinery that comes with `apaquarto-pdf` |
| `gfm` and other plain markdown | none; Quarto does not build figures and tables there for a note to attach to |
