...
R draws each figure through a graphics device, and apaquarto names one for
every format so that the same document draws the same way everywhere:

| Format | Device | R package |
|--------|--------|-----------|
| `apaquarto-html` | `svglite` | svglite |
| `apaquarto-typst` | `svglite` | svglite |
| `apaquarto-docx` | `ragg_png` | ragg |
| `apaquarto-pdf` | `pdf` | (built in) |

So a document with R chunks needs the svglite and ragg packages:

``` r
install.packages(c("svglite", "ragg"))
```

These are chosen partly to keep clear of R's own `svg()` and `png()` devices,
which draw through cairo. On macOS cairo comes from XQuartz, and on a machine
without it a render stops at the first chunk that draws anything:
...
followed by a long list of libraries it could not load from `/opt/X11`. If you
see this, check that svglite and ragg are installed. A document in a plain
Quarto format rather than an apaquarto one is on R's cairo devices and can hit
the same wall; naming a device in the document settles it:
...
### Notes in Formats Outside of apaquarto Documents

A user might want a note below a figure in a document that is not otherwise in APA-style. The apaquarto extension has the `apanote` filter that can add table notes to a non-apaquarto Quarto document. For example: 
...
The `quarto` line is part of it. A filter named on its own runs before Quarto builds its figures and tables, so there would be nothing for a note to attach to; naming `quarto` first puts `apanote` after that work, which is where it belongs.

`apanote` is a small companion extension that is
installed alongside apaquarto and names a few of its filters. Nothing else of apaquarto comes with them: no title page, no APA headings, no stylesheet. The note is written in place below its figure or table and takes whatever styling the document already has.

Notes on figures and tables written in code chunks, on markdown images, and on markdown tables are all read. Which of them survive depends on the format:

| Format | A figure | A table | A figure laid out in panels |
|--------|----------|---------|-----------------------------|
| `html`, `revealjs` | yes | yes | each panel's own note, but not the note of the whole figure |
| `docx` | yes | yes | each panel's own note, but not the note of the whole figure |
| `typst` | yes | no | no |
| `pdf` | no | no | no |
| `gfm` and other plain markdown | no | no | no |

The gaps are not arbitrary. A note belonging to a whole figure of panels, rather
than to any one panel, is written by a filter that runs at a later point in the
render than the ones `apanote` names, and `apanote` has no way of naming that
point: an extension that lists its filters as plain paths, which is what every
Quarto before 1.9 can read, gets to choose only between running before Quarto's
own work and after it. In typst a note on a markdown table is lost because
Quarto's typst writer has turned the table into typst before the filter runs,
and a figure of panels is built by apaquarto's own typst code, which is not part
of `apanote`. In .pdf the notes are set by the LaTeX machinery that comes with
`apaquarto-pdf`, and in plain markdown Quarto builds no figures or tables for a
note to attach to.

All of these work in full in apaquarto's own formats. `apanote` is for a
document that is not otherwise in APA style; a document that is, should name an
apaquarto format instead.
...
## Tables and Figures Spanning Two Columns in Journal Mode (.pdf and Typst)

When creating tables and figures in journal mode, care must be taken not to make figures and tables wider than the columns, otherwise \LaTeX sometimes makes them disappear (and sometimes takes large swaths of text with it). Typst output handles wide floats more gracefully, but the same option applies.

You can make figures and tables span the two columns by setting the `apa-twocolumn` chunk option to `true`. This affects .pdf and Typst documents when in journal mode (i.e., `documentmode: jou`).
...
::: callout-warning
This "escaped backslash in front of the semicolon trick" is a hack. When you switch to Visual Mode in RStudio or Positron, it thinks you made a mistake, and it removes the backslash. If the citation above looks wrong, it is because I forgot to fix it after switching to visual mode and back. I reported this bug to the RStudio/Positron developers at Posit. I hope the bug will be fixed soon.
:::

### Suppressing Parenthetical Ampersands

Strict APA style uses ampersands in parenthetical citations with multiple authors. If you want the word "and" instead, add this field to your YAML header:

``` yaml
no-ampersand-parenthetical: true
```
If you are not writing in English, you can set "and" to whatever is appropriate. For example, "and" in Spanish is "y", which can be set like so:
...
Typically, a plain citation without any brackets works without any fuss:
...
`@CameronTrivedi2013 [101-103] said some interesting things. So did @cohen2003applied [21].`

@CameronTrivedi2013 [101-103] said some interesting things. So did @cohen2003applied [21].

You do not not need to add "p." or "pp." Quarto will get it right automatically. A hyphen (-) between two page numbers will automatically be converted to an en dash (--).

### Possessive Citations

A useful feature specific to apaquarto is automatic "possessive citations."

`@CameronTrivedi2013 ['s] position` becomes @CameronTrivedi2013 ['s] position.

You can add page numbers with possessive citations. `@CameronTrivedi2013 ['s, 35] idea was great. @CameronTrivedi2013 ['s, 124-125] other idea was good, too.`  becomes @CameronTrivedi2013 ['s, 35] idea was great. @CameronTrivedi2013 ['s, 124-125] other idea was good, too.

If you render your document in another format, you would need to supply the name in text like this: `Cameron and Trivedi's [-@CameronTrivedi2013] position`, which becomes Cameron and Trivedi's [-@CameronTrivedi2013] position. This works because putting a hyphen before the reference suppresses the author name. That is, `[-@CameronTrivedi2013]` becomes [-@CameronTrivedi2013].

### Narritive Citations with Ampersands

If you have narrative citation in a table, APA Style encourages the use of ampersands to save space (Parenthetical citations already have ampersands.). To suppress the conversion of ampersands to "and", place an ampersand in brackets after the citation:

`@CameronTrivedi2013 [&] said something` becomes @CameronTrivedi2013 [&] said something.

This works with possessive citations and/or page numbers:

`@CameronTrivedi2013 [&, 's] idea is interesting` becomes @CameronTrivedi2013 [&, 's] idea is interesting.

The order of these tags is unimportant, but they must be separated by a space. The comma is optional.

`@CameronTrivedi2013 ['s &] idea is interesting` becomes @CameronTrivedi2013 ['s &] idea is interesting.

Put page numbers in the last position, separated by a space. The comma is optional.

`@CameronTrivedi2013 [&, 35--36] have an interesting idea` becomes @CameronTrivedi2013 [&, 35--36] have an interesting idea.

`@CameronTrivedi2013 [&, 's, 35--36] idea is interesting` becomes @CameronTrivedi2013 [&, 's, 35--36] idea is interesting.
...
If the `mask` field is set to `false`, the masked citations will print as usual. When `mask` is `true`, depending if they are narrative or parenthetical citations, they will be listed as "Masked Citations (n.d)" or "(Masked Citations, n.d)". In parenthetical citations, it is possible to mix masked and unmasked citations.

Setting `mask` to `true` will also remove the authors, affiliations, and author notes from the title page.

The text for masked citations have these defaults:
...
They can be changed to another language or alternate text in English in your YAML header. For example:

``` yaml
language:
  citation-masked-author: "Author masked for review"
  citation-masked-date: "Date masked"
  citation-masked-title: "Title masked for review"
```
