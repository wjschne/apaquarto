...
# Version 6.0.0
- 2026-09-12
- apaquarto-pdf is built without the apa7 class. The title page, the headings, the running head and the notes below floats arrive as ordinary blocks from the same filters that build the .html, the .docx and the typst, and latex is asked only to set them; what was a class to argue with is now a page of style definitions, and Quarto's own pdf template is used unmodified. This began as a second format, apaquarto-latex-pdf, and replaced the apa7 one before the release rather than shipping beside it, so there is still a single pdf format and nothing to choose between. Twelve files went with apa7: the template and its three partials, and the eight filters that existed to persuade the class to do what the other formats do for themselves (credit to Claude).
