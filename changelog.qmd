...
# Version 6.0.0
- 2026-09-12
- apaquarto-typst draws R figures with svglite, as apaquarto-html already did. Quarto leaves typst on R's own `svg()` device, which draws through cairo, and cairo on macOS is part of XQuartz; on a machine without it there is no working svg device and the render stopped at the first chunk that drew anything, with `dev.control() called without an open graphics device` under a list of libraries it could not load from `/opt/X11`. svglite writes the same picture without cairo, and its text stays text in the .pdf rather than being flattened to outlines. typst renders now need the svglite package, which apaquarto-html already needed (credit to Claude).
...
- [#158](https://github.com/wjschne/apaquarto/issues/158) The `apa-note` of a figure embedded from a notebook is written in the article again. A manuscript project renders the notebook on its own before the article embeds a cell out of it, and the mark apaquarto leaves on a float whose note it has already written travelled into the article with the cell --- while the note stayed behind, since only the cell's output is embedded. The article therefore skipped a note it had never written. The mark now names the document it was made for, so a mark from the notebook no longer silences the article, while a document that runs the note filters twice, by naming `apanote` in `filters:` alongside an apaquarto format, still prints each note once (credit to Claude).
- [#158](https://github.com/wjschne/apaquarto/issues/158) `apa-note` can be used in a document that is not in an apaquarto format. Adding `filters:` with `apanote` under it to an ordinary document brings in the filters that read a note and write it below its figure or table, and nothing else of apaquarto --- no title page, no APA headings, no stylesheet. `apanote` is a small companion extension that installs alongside apaquarto and names a few of its filters, so `quarto add wjschne/apaquarto` brings both. It covers .html, revealjs and .docx in full and typst for figures; .pdf still needs `apaquarto-pdf`, whose notes are built by the LaTeX template. A document in an apaquarto format that names the filters as well now prints each note once rather than twice (credit to Claude).
...
- The typst jou masthead is lifted into the top margin by half of it, so that the head of the first page sits about where a journal puts it. Only the first page moves: the pages after it keep the margin, and the running head in it, that the mode gives. A document that sets its own `margin` is lifted by half of that.
- In typst jou mode, the first line of the author note is given 3 more points of clearance under the rule that separates it from the body, which its ascenders had very nearly been touching.
...
- In typst jou mode, an author note too long for one column now runs across the foot of the page in two columns, the way a journal sets a long author note, instead of filling the first column and leaving the article to start in what little was left above it. A note of ordinary length is unchanged: it stays at the foot of the first column. The turning point is a third of the column.
...
- `documentmode` now accepts the modes spelled out in full, in both apaquarto-pdf and apaquarto-typst: `journal`, `manuscript`, `document`, and `student` are aliases for `jou`, `man`, `doc`, and `stu`.
...
- [#162](https://github.com/wjschne/apaquarto/pull/162) `documentmode` now works in apaquarto-typst, with all four modes: `man`, `jou` (two columns, with column-spanning floats), `doc`, and `stu`. Thanks [David Hagmann](https://github.com/wjschne/apaquarto/pull/162)
...
- Typst jou mode now carries a journal running head: the short title centred on odd pages, the author surnames centred on even ones, both in 8 point capitals, with the page number in the outer corner (left on even pages, right on odd). The opening page carries neither. The surnames take serial commas and an ampersand, and become "et al." after the first author once there are more than three of them.
...
- In typst jou mode, the running head sits 12 points clear of the text block rather than at the foot of the margin.
- Level 1 to 3 headings are now set ragged right with hyphenation off, so a heading that runs to two lines in typst jou mode is neither stretched to the column edge nor broken across lines mid-word.
- In typst jou mode, the impact statement is set in a 1 point box with 5 points of padding, as wide as the abstract above it, with 9 points of space between the box and the abstract above and the keywords below.
- In typst jou mode, block quotations follow apa7 more closely: set 2 points smaller than the body, with the paragraph that opens the quotation flush left and the ones after it indented, the paragraphs separated by nothing more than the line spacing, and 9 points of space between the quotation and the text around it.
- In typst jou mode, a figure or table title ("Figure 1", "Table 1") stands at least 9 points clear of whatever is above it, rather than taking the body's tighter space between paragraphs.
