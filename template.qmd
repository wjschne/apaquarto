...
#| apa-note: "*g*&nbsp;=&nbsp;general ability true score"
#| out-width: "100%"
plot(1:10)
```

To refer to any figure or table, put the chunk label in curly braces. For example, see {apafg-myplot}. 

The easiest way to import an existing graphic as a figure is to use `knitr::include_graphics` in a code chunk. For example, {apafg-importedgraphic} is an imported image. Note that in apaquarto-pdf documents, we can specify that that a figure or table should span both columns when in journal mode by setting the `apa-twocolumn` chunk option to `true`. For other formats, this distinction does not matter.

```{r apafg-importedgraphic}
...
To refer to this table in text, put the table's reference label in curly braces like so: As seen in {apatb-mytable}, the first few numbers and letters of the alphabet are displayed.
