...
library(gt)
mtcars %>% 
  mutate(Engine = factor(vs, labels = c("V-Shaped", "Straight")),
         Transmission = factor(am, labels = c("Automatic", "Manual"))) %>% 
  count(Engine, Transmission) %>% 
  pivot_wider(names_from = Transmission, 
              names_prefix = "Transmission_", 
              values_from = n) %>% 
  gt() %>% 
  tab_spanner_delim(delim = "_") %>% 
  gt::cols_align("left", columns = "Engine") %>% 
  apa7::apa_style() %>% 
  gt::tab_options(table.font.names = "Times New Roman") %>% 
  as_word() %>% 
  # gsub(pattern = "</w:tbl>",replacement = "</w:tbl><w:p></w:p>", fixed = TRUE)
  gsub(pattern = "<w:rFonts w:ascii=\"Calibri\" w:hAnsi=\"Calibri\"/>\n", replacement = "", fixed = TRUE) %>% 
  gsub(pattern = "<w:spacing w:before=\"0\" w:after=\"60\"/>\n ", replacement = "", fixed = TRUE) %>% 
  knitr::asis_output(x = paste("\n\n``````{=openxml}", ., 
            "``````\n\n", sep = "\n")) %>% 
  knitr::knit_print(inline = F)
