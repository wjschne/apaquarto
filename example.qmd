...
keywords: 
  ...
  - Markdown
  - apaquarto
# Optional pointer to supplemental materials, shown on the line below the keywords.
supplemental-materials: "Documentation and web app at  <https://wjschne.github.io/apaquarto/>"
# Optional statement about the importance of the paper. The heading can be customized in the language field.
impact-statement: "The apaquarto template allows writers to apply APA style consistently with minimal fuss and bother."
...
language:
  ...
  title-supplemental-materials: "Supplemental materials"
...
format:
  ...
  apaquarto-typst: 
    # Options: manuscript (default), journal, student, document
...
# flextable names its font in the output it writes, and a pdf built with
# lualatex asks fontspec for that name: {\fontspec{Times New Roman} ...}
# around every cell. Times New Roman ships with windows and with office on
# a mac, and is on no linux box by default, where fontspec stops the build
# rather than substituting -- "The font Times New Roman cannot be found".
# TeX Gyre Termes is the times metric-compatible face that comes with every
# tex distribution, so the pdf asks for that and looks the same. Word and a
# browser want the real name, and have it.
table_font <- if (knitr::is_latex_output()) "TeX Gyre Termes" else "Times New Roman"
set_flextable_defaults(theme_fun = theme_apa, 
                       font.family = table_font, 
                       text.align = "center", 
                       table_align = "left")
# apa7 keeps a font of its own, which apa_flextable, apa_chisq and apa_cor
# take when they are not given one. set_flextable_defaults does not reach it.
apa7_defaults(font_family = table_font)
```
...
#| fig-height: 2
#| fig-width: 3
#| fig-alt: A positively skewed log-normal distribution.
...
Quarto supports multiple panels in a figure, and thus apaquarto does as well. 

:::{#fig-multipanel layout-ncol=2 fig-cap="Two Distributions" apa-note="A normal distribution has no bounds, whereas a beta distribution ranges from 0 to 1."}
...
#| fig-height: 2
#| fig-width: 3
#| fig-alt: A normal distribution.
#| apa-note: "NoNote Mean = 0, SD = 1"
...
```{r}
#| label: subpanelb
#| fig-cap: "A Beta Distribution"
#| fig-alt: A beta distribution.
#| fig-height: 2
#| fig-width: 3
#| apa-note: "NoNote $\\alpha=0, \\beta = 1$"
...
However, you might find that perfect APA style may require fine-tuning that is better achieved using the patchwork package [@pedersenPatchworkComposerPlots2025], as in @fig-patchwork.

```{r}
#| label: fig-patchwork
#| fig-cap: A multipanel plot using patchwork
#| fig-height: 2
#| apa-note: "Not only do statistical distributions have distinct shapes, they are used for diverse purposes. Panel A: Because a Cauchy distribution has a location but no mean and no variance, it has thick tails, making it useful when modeling variables with extreme events. Panel B: A Weibull distribution (shown here with shape = 2) is used to model time between events."
library(patchwork)
p1 <- ggplot(data.frame(x = c(-20, 20)), aes(x)) +
...
p2 <- ggplot(data.frame(x = c(0, 3)), aes(x)) +
...
#| apa-note: Figures in two-column mode are only different for journal mode in .pdf and typst documents
#| apa-twocolumn: true
#| fig-height: 3
#| fig-width: 6.2
#| fig-align: center
...
: Classic Scale Types. {#tbl-mymarkdowntable2 apa-note="My note"}
...
![Appendix Figure](sampleimage.png){#fig-appendfig apa-note="A *note* below the figure" fig-alt="A path diagram in which A causes B, and B causes C."}
...
@apx-b is an appendix with a contingency table (see @tbl-chisq).
...
mtcars |>
  ...
  apa_chisq(table_width = .45, font_size = 10)
```
