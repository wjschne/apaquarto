...
We can make a table the same way as a figure. Generating a table that conforms to APA format in all document formats can be tricky. When the table is simple, the `kable` function from knitr works well. Feel free to experiment with different methods, but I have found David Gohel's [flextable](https://davidgohel.github.io/flextable/) to be the best option when I need something more complex.
...
@apx-a is an appendix with a table using flextable (see @tbl-flextable).

```{r}
#| label: tbl-flextable
#| tbl-cap: Demonstration of flextable functions
...
mtcars |>
  ...
  count(Transmission, Gears, .drop = FALSE) |>
  mutate(p = scales::percent(n / sum(n)), .by = Transmission) |>
  pivot_wider(
    names_from = Transmission,
    values_from = c(n, p),
    names_glue = "{Transmission}_{.value}",
    names_vary = "slowest"
  ) |>
  flextable() |>
  separate_header() |>
