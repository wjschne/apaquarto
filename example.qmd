...
If you want the tables and figures to be at the end of the document? In the .pdf format, you can set the `floatsintext` option to false. The reference labels will work no matter where they are in the text.
...
# Appendix {#sec-appendixa}

# My Appendix Title

If there are multiple appendices, label them with level 1 headings as Appendix A, Appendix B, and so forth. Tables and figures in the first appendix automatically get the prefix "A", and the numbering starts again at 1. See @fig-appendfig, an example of an imported graphic using markdown syntax. If there were a second appendix, tables and figures would get the prefix "B", and the numbering starts again at 1. Make as many appendices as needed.

To cite an appendix as a whole, place a section identifier with a `sec-` prefix beside the Appendix heading, as shown in this document. Then, cite that identifier in the text. For example, see @sec-appendixa.
