...
apaquarto needs **Quarto 1.9 or greater**. The .pdf and Typst outputs rely on parts of Quarto that earlier versions do not have, and installing apaquarto on an older Quarto stops with a message saying so.

If you use a recent version of [RStudio](https://posit.co/download/rstudio-desktop/), Quarto is already installed --- but RStudio carries whichever version it was built with, which may be older than 1.9. The check below says which version is being used; if it is older than 1.9, install Quarto on its own from the link above and restart RStudio.
...
An alternative that is a little easier for me to remember, my [apa7](https://wjschne.github.io/apa7/) package has a dedicated function for installing apaquarto:

```{r}
#| eval: false
apa7::install_apaquarto(no_prompt = TRUE)
```
